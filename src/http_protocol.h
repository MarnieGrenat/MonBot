...
void sendAlert(String message)
{
    ...
    if (phoneNumber == "" || apiKey == "")
    {
        ...
        phoneNumber = apiConfig["phoneNumber"].as<String>();
    ...
    switch(responseCode)
    {
        case HTTP_CODE_OK:
            ...
            break;
        case HTTP_CODE_NON_AUTHORITATIVE_INFORMATION:
            ESP_LOGE("ALERT", "API Key or Phone number is invalid.");
            break;
        default:
            ESP_LOGE("ALERT", "Unexpected response code: %u", responseCode);
            break;
    }
    http.end();
}
...
/**
 * @brief Deve ser chamado com frequência mínima de 24hrs para garantir sistema com horário atualizado.
 *
 */
void reconfigTime()
{
