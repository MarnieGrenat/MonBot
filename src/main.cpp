...
void sendMQTTMessage(const char* topic, const char* payload, uint8_t qos = 0);
void treatReceivedMessage(String &topic, String &payload);

void taskInternOperations(void*);
...
void setup() {
  ...
  ESP_LOGV("SETUP", "Loading System.");
  while (!beginFileSystem())
  {
    taskWait(1000);
  }
  presentDevice();
  ...
  ESP_LOGV("SETUP", "Loading basic functionalities.");
  loadDeviceSettings();
  ESP_LOGV("SETUP", "Loading basic functionalities.");
  taskWait(1000);
  initWifi();
  ...
  ESP_LOGI("SETUP", "%s", WiFi.localIP().toString());

  taskWait(1000);
  xTaskCreate(taskInternOperations, "Intern Operations", 10 * 1024, NULL, 2, &taskHandle_Control);
  taskWait(10*1000);
...
void taskMQTT(void*)
{
  ...
  for(ever)
  {
    ...
    if (!isConnectedToMQTT)
    {
      ...
      { // Cuidado com taskWaits, evitando explodir keepalive.
...
void sendMQTTMessage(const char* topic, const char* payload, uint8_t qos)
{
  isConnectedToMQTT ? mqttClient->publish(topic, payload, false, qos) :
  ESP_LOGE("MQTT PUBLISH", "Device not Connected to Broker.");
}
